...
checked by `src/architecture.test.ts` — if you break one, that test tells you
which import did it and why the rule exists. An architecture nobody can violate
...
they meet at `shared/ipc.ts`, which is the only description of what may cross.

**`src/core` is the important one.** It runs in a bare Node process, so its
tests need no Electron, no browser and no build step: 30-odd modules, near one
test file each, and the whole suite is seconds. That is why logic belongs there
...
| to coordinate several of the above      | a store slice — thin         |

**A store slice is orchestration, not logic.** It calls IPC, sets state, and
decides nothing on its own. When a slice starts making decisions — which tab to
activate next, how to rearrange panes — that decision is a pure function over a
plain value, and it goes in `core/`. `core/tab-layout.ts` is the worked example:
it came out of a `set()` callback where none of its rules could be tested.
...
Excalidraw is a built-in that uses nothing a community plugin could not.

## Testing

- **`core/`** — unit tests, one file each, no mocks worth the name.
- **`main/services/`** — unit tests against real temp directories and real git.
- **`renderer/`** — behaviour goes through Playwright against the built app;
  unit tests cover the editor extensions that can be driven headlessly.
